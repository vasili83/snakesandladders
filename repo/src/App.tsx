...
function App() {
  ...
  useEffect(() => {
    youWin(playerpos) && setWinlosemssg("Gewonnen! Je bent precies op vak 36 gekomen!");
  ...
  const checkLaddersOrSnakes = (rolledDiceSum: number): number => {
    ...
    const moveUp = (): number | undefined => {
      ...
      if (arrPair) {
        setWinlosemssg("Super! Je hebt een ladder genomen");
        return Math.max(...arrPair)
      } else {
        return undefined;
      }
    }
    ...
    const moveDown = (): number | undefined => {
      ...
      if (arrPair) {
        setWinlosemssg("Jammer, een slang die je stappen terug zet");
        return Math.min(...arrPair);
  ...
  const movePlayer = (rolledDiceSum: number) => {
    ...
    if (newPos < 37) {
      ...
      setWinlosemssg('Overshoot! Je blijft staan op veld staan');
    }
  }
  ...
  return (
    <>
      ...
      <main>
        <h2 className="mssg">{winlosemssg}</h2>
        <h3>Speler positie: {playerpos}</h3>
        <p>Dobbelsteen: <span className="dice">{dice1 !== 0 && diceSymb[dice1 - 1]}</span> | Dobbelsteen: <span className="dice">{dice2 !== 0 && diceSymb[dice2 - 1]}</span></p>
        <p>Totaal dobbelstenen: {diceSum}</p>
